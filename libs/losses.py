...
def merge_loss(outputs, targets, weights=[0.25, 0.75]):
    ...
    if torch.isnan(dl2):
        ...
        cprint(
            "WARNING: dl2_loss is nan. Resetting it to 1e-8", "yellow", attrs=["bold"],
        )

    if torch.isnan(dl3):
        dl3.data = torch.tensor(1e-8).data
        cprint(
            "WARNING: dl3_loss is nan. Resetting it to 1e-8", "yellow", attrs=["bold"],
        )

    if torch.isnan(rl2):
        rl2.data = torch.tensor(1e-8).data
        cprint(
            "WARNING: rl2_loss is nan. Resetting it to 1e-8", "yellow", attrs=["bold"],
        )

    if torch.isnan(rl3):
        rl3.data = torch.tensor(1e-8).data
        cprint(
            "WARNING: rl3_loss is nan. Resetting it to 1e-8", "yellow", attrs=["bold"],
        )
